...
export function ProjectDetailPage() {
  ...
  const connectRepository = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const data = new FormData(form)
    try {
      await api.createRepository(projectId, {
        name: String(data.get('name')),
        localPath: String(data.get('localPath')),
        initialize: data.get('initialize') === 'on',
      })
      form.reset()
      repositories.reload()
    } catch (value) {
  ...
  return (
    <Page>
      ...
      <div className="page-title">
        <div>
          <span className="eyebrow">Project / {project.data?.slug}</span>
          <h1>{project.data?.name}</h1>
          <p>{project.data?.description || 'No project description.'}</p>
        </div>
        <Link className="button primary" to={`/workflows/new?project=${projectId}`}>
          ＋ New workflow
        </Link>
      </div>
      {actionError && (
      ...
      <div className="two-column">
        <section className="panel panel-stack">
          ...
          <form className="compact-form" onSubmit={createTask}>
            ...
            <label>
              Description
              <textarea name="description" rows={2} maxLength={10000} />
            </label>
            <button className="button primary" disabled={busy === 'task'}>
          ...
          {tasks.loading ? (
              ...
              {tasks.data.items.map((task) => (
                <div className="row" key={task.id}>
                  ...
                  <select
                    ...
                    onChange={(event) => void updateTask(task, event.target.value as Task['status'])}
                  >
            ...
            <EmptyState title="No tasks" body="Add the first task above." />
          )}
        </section>
        <section className="panel panel-stack">
          <div className="panel-head">
          ...
          <form className="compact-form" onSubmit={connectRepository}>
            <label>
              Name
            ...
            <label>
              Workspace path
              <input name="localPath" placeholder={project.data?.slug || 'my-project'} required />
              <small>Use a relative path to create a repository managed by ForgeFlow.</small>
            </label>
            <label className="check">
              <input name="initialize" type="checkbox" defaultChecked /> Initialize a managed Git repository
            </label>
            <button className="button primary" disabled={busy === 'repository'}>
          ...
          {repositories.loading ? (
            ...
            <ErrorState error={repositories.error} retry={repositories.reload} />
            ...
            <EmptyState
              ...
              body="Create a managed repository above or connect an existing checkout in the workspace volume."
            />
          )}
        </section>
      </div>
      <div className="two-column">
        <section className="panel panel-stack">
          <div className="panel-head">
            ...
            <Link to={`/workflows/new?project=${projectId}`}>Create →</Link>
          </div>
          {workflows.loading ? (
            <LoadingState />
          ) : workflows.error ? (
            <ErrorState error={workflows.error} retry={workflows.reload} />
              ...
              {workflows.data.items.map((workflow) => (
                <div className="row" key={workflow.id}>
                  <div>
                    ...
                    <small>
                      {workflow.steps.length} step{workflow.steps.length === 1 ? '' : 's'} · version {workflow.version}
                    </small>
                  </div>
                  <button className="button secondary small" disabled={busy === workflow.id} onClick={() => void runWorkflow(workflow.id)}>
                    {busy === workflow.id ? 'Starting…' : 'Run'}
                  </button>
                </div>
              ))}
            </div>
            ...
            <EmptyState
              title="No workflows"
              body="Create a workflow to automate a repeatable check."
              action={
                <Link className="button secondary" to={`/workflows/new?project=${projectId}`}>
                  Create workflow
                </Link>
              }
            />
        ...
        <section className="panel panel-stack">
          ...
          {runs.loading ? (
            ...
            <EmptyState title="No workflow runs" body="Start a workflow and its result will appear here." />
          )}
        </section>
      </div>
    </Page>
