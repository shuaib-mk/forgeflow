...
export function DashboardPage() {
  ...
  const analytics = useAsync(() => api.analytics(organizationId), [organizationId])
  const projects = useAsync(() => api.projects(organizationId), [organizationId])
  if (!organizationId)
    return (
      <Page>
        <EmptyState title="No organization available" body="Create an account with an organization to load a workspace." />
      </Page>
    )
  return (
    <Page>
      <div className="page-title">
        <div>
          ...
          <p>Here’s what is moving across your workspace.</p>
        </div>
      ...
      {analytics.loading ? (
        ...
        <ErrorState error={analytics.error} retry={analytics.reload} />
      ) : (
        <div className="metric-grid">
          {Object.entries(analytics.data ?? {}).map(([key, value]) => (
            <article className="metric" key={key}>
              ...
              <strong>{value}</strong>
              <small>{key === 'failedRuns' && value > 0 ? 'Needs attention' : 'Current total'}</small>
            </article>
          ))}
        </div>
      )}
      <section className="panel">
        ...
        {projects.loading ? (
          ...
          <EmptyState title="No projects yet" body="Create a project to connect a repository and run your first workflow." />
        )}
      </section>
    </Page>
  )
