...
export function LoginPage() {
  ...
  return <main className="auth-page">
    <section className="auth-copy">
      <a className="brand light" href="/"><span className="brand-mark">F</span>Forge<span>Flow</span></a>
      <div><span className="eyebrow">Local by design</span><h1>Ship better software,<br /><em>one flow at a time.</em></h1><p>Projects, quality checks, test runs, and engineering history in one dependable workspace.</p></div>
    ...
    <section className="auth-form-wrap">
      <form className="auth-form" onSubmit={submit}>
        ...
        {mode === 'register' && <>
          <label>Your name<input name="displayName" autoComplete="name" maxLength={100} required /></label>
          <label>Organization name<input name="organizationName" maxLength={100} required /></label>
          <label>Organization slug<input name="organizationSlug" placeholder="my-team" pattern="[a-z0-9]+(-[a-z0-9]+)*" required /><small>Lowercase letters, numbers, and single hyphens.</small></label>
        </>}
        ...
        <button className="button primary wide" disabled={submitting}>{submitting ? 'Please wait…' : mode === 'login' ? 'Sign in' : 'Create account'}</button>
      </form>
    </section>
  </main>
}
