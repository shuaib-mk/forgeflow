...
export type Project = { id: string; organizationId: string; name: string; slug: string; description: string; createdAt: string; updatedAt: string }
export type Task = { id: string; projectId: string; title: string; description: string; status: 'open' | 'in_progress' | 'done' | 'canceled'; createdAt: string }
export type Repository = { id: string; projectId: string; name: string; localPath: string; defaultBranch: string }
export type WorkflowRun = { id: string; workflowId: string; projectId: string; status: 'queued' | 'running' | 'succeeded' | 'failed' | 'canceled' | 'timed_out'; createdAt: string; error?: string }
