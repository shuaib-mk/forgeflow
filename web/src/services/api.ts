import type { Analytics, ApiFailure, AuditEvent, Organization, OrganizationMembership, Page, Plugin, Project, Repository, Task, User, Workflow, WorkflowRun } from '../types/models'
...
export const api = {
  register: (input: { email: string; displayName: string; password: string; organizationName: string; organizationSlug: string }) => request<{ user: User; organization: Organization }>('/api/v1/auth/register', { method: 'POST', body: JSON.stringify(input) }),
  ...
  createTask: (projectId: string, input: { title: string; description: string }) => request<Task>(`/api/v1/projects/${encodeURIComponent(projectId)}/tasks`, { method: 'POST', body: JSON.stringify(input) }),
  updateTask: (projectId: string, taskId: string, status: Task['status']) => request<Task>(`/api/v1/projects/${encodeURIComponent(projectId)}/tasks/${encodeURIComponent(taskId)}`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  repositories: (projectId: string) => request<{ items: Repository[] }>(`/api/v1/projects/${encodeURIComponent(projectId)}/repositories`),
  createRepository: (projectId: string, input: { name: string; localPath: string; initialize?: boolean }) => request<Repository>(`/api/v1/projects/${encodeURIComponent(projectId)}/repositories`, { method: 'POST', body: JSON.stringify(input) }),
  workflows: (projectId: string) => request<{ items: Workflow[] }>(`/api/v1/projects/${encodeURIComponent(projectId)}/workflows`),
  createWorkflow: (projectId: string, definition: unknown) => request<Workflow>(`/api/v1/projects/${encodeURIComponent(projectId)}/workflows`, { method: 'POST', body: JSON.stringify(definition) }),
  runWorkflow: (workflowId: string) => request<WorkflowRun>(`/api/v1/workflows/${encodeURIComponent(workflowId)}/runs`, { method: 'POST' }),
  runs: (organizationId: string, projectId = '') => request<Page<WorkflowRun>>(`/api/v1/runs?organizationId=${encodeURIComponent(organizationId)}&projectId=${encodeURIComponent(projectId)}`),
