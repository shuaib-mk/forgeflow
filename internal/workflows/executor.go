...
func (r LocalCommandRunner) Run(ctx context.Context, workingDirectory string, step models.WorkflowStep, output io.Writer) error {
	...
	if err := command.Run(); err != nil {
		return fmt.Errorf("execute %s: %w", step.Command, err)
	}
	return nil
}
...
func (e Executor) Execute(ctx context.Context, definition Definition, workingDirectory string) Result {
	if e.Runner == nil {
		return Result{Status: models.RunFailed}
	}
	backoff := e.Backoff
	if backoff == nil {
		backoff = func(attempt int) time.Duration { return time.Duration(1<<uint(attempt-1)) * time.Second }
	}
	...
	for _, step := range definition.Steps {
		...
		for _, dependency := range step.DependsOn {
			if statuses[dependency] != models.RunSucceeded {
				blocked = true
				break
			}
		}
		...
		if stepResult.Status != models.RunSucceeded && !step.ContinueOnFail {
...
func (e Executor) executeStep(ctx context.Context, directory string, step models.WorkflowStep, backoff func(int) time.Duration) StepResult {
	...
	for attempt := 1; attempt <= step.Retries+1; attempt++ {
		...
		if errors.Is(contextErr, context.DeadlineExceeded) {
			result.Status = models.RunTimedOut
		} else if errors.Is(contextErr, context.Canceled) {
			result.Status = models.RunCanceled
		}
