...
func NewService(repository Repository, bus *events.Bus) *Service {
	return &Service{repository: repository, events: bus, now: time.Now}
}

type CreateInput struct {
	OrganizationID models.ID `json:"organizationId"`
...
func (s *Service) Create(ctx context.Context, actor models.User, input CreateInput) (models.Project, error) {
	role, err := s.repository.Role(ctx, input.OrganizationID, actor.ID)
	if err != nil {
		return models.Project{}, err
	}
	if role != models.RoleOwner && role != models.RoleAdmin && role != models.RoleDeveloper {
		return models.Project{}, domain.ErrForbidden
	}
	if !validation.Required(input.Name, 100) {
		return models.Project{}, domain.Invalid("name", "must contain 1 to 100 characters")
	}
	if !validation.Slug(input.Slug) {
		return models.Project{}, domain.Invalid("slug", "must be a lowercase URL-safe slug")
	}
	if len(input.Description) > 2000 {
		return models.Project{}, domain.Invalid("description", "must not exceed 2000 characters")
	}
	now := s.now().UTC()
	project := models.Project{ID: models.ID(uuid.NewString()), OrganizationID: input.OrganizationID, Name: strings.TrimSpace(input.Name), Slug: input.Slug, Description: strings.TrimSpace(input.Description), CreatedBy: actor.ID, CreatedAt: now, UpdatedAt: now}
	if err := s.repository.Create(ctx, project); err != nil {
		return models.Project{}, err
	}
	if s.events != nil {
		_ = s.events.Publish(ctx, events.ProjectCreated{ProjectID: project.ID, ActorID: actor.ID, At: now})
	}
	return project, nil
}
...
func (s *Service) Get(ctx context.Context, actor models.User, id models.ID) (models.Project, error) {
	...
	if _, err := s.repository.Role(ctx, project.OrganizationID, actor.ID); err != nil {
		return models.Project{}, err
	}
	return project, nil
}
...
func (s *Service) List(ctx context.Context, actor models.User, input ListInput) (models.Page[models.Project], error) {
	if _, err := s.repository.Role(ctx, input.OrganizationID, actor.ID); err != nil {
		return models.Page[models.Project]{}, err
	}
	if input.Page < 1 {
		input.Page = 1
	}
