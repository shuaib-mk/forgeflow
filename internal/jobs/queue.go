...
type Queue interface {
	...
	Dequeue(context.Context, time.Duration) (models.ID, error)
}
...
func (q *RedisQueue) Enqueue(ctx context.Context, id models.ID) error {
	if err := q.client.LPush(ctx, queueKey, string(id)).Err(); err != nil {
		return fmt.Errorf("enqueue workflow run: %w", err)
	}
	return nil
}
func (q *RedisQueue) Dequeue(ctx context.Context, timeout time.Duration) (models.ID, error) {
	result, err := q.client.BRPop(ctx, timeout, queueKey).Result()
	if errors.Is(err, redis.Nil) {
		return "", context.DeadlineExceeded
	}
	if err != nil {
		return "", fmt.Errorf("dequeue workflow run: %w", err)
	}
	return models.ID(result[1]), nil
}
