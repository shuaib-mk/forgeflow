...
func TestAuthorizationMatrix(t *testing.T){
	t.Parallel()
	for _,tt:=range []struct{role models.Role;permission Permission;allowed bool}{
	...
	}{if got:=Require(tt.role,tt.permission)==nil;got!=tt.allowed{t.Errorf("Require(%s,%s) allowed=%v",tt.role,tt.permission,got)}}
}
