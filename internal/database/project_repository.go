...
type ProjectFilter struct { OrganizationID models.ID; Search string; Sort string; Desc bool; Limit, Offset int }
...
func (r *ProjectRepository) Create(ctx context.Context, project models.Project) error {
	...
	if err != nil { return fmt.Errorf("create project: %w", err) }
	return nil
}
...
func (r *ProjectRepository) Get(ctx context.Context, id models.ID) (models.Project, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return models.Project{}, domain.ErrNotFound }
	if err != nil { return models.Project{}, fmt.Errorf("get project: %w", err) }
	return project, nil
}

func (r *ProjectRepository) List(ctx context.Context, filter ProjectFilter) ([]models.Project, int, error) {
	order := "created_at"
	if filter.Sort == "name" { order = "name" }
	direction := "ASC"
	if filter.Desc { direction = "DESC" }
	...
	for rows.Next() {
		var project models.Project
		if err := rows.Scan(&project.ID, &project.OrganizationID, &project.Name, &project.Slug, &project.Description, &project.CreatedBy, &project.CreatedAt, &project.UpdatedAt, &total); err != nil { return nil, 0, fmt.Errorf("scan project: %w", err) }
...
func (r *ProjectRepository) Role(ctx context.Context, organizationID, userID models.ID) (models.Role, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return "", domain.ErrForbidden }
	if err != nil { return "", fmt.Errorf("get membership: %w", err) }
	return role, nil
}
