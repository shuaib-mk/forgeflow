...
func (r *AuthRepository) Register(ctx context.Context, user models.User, organization models.Organization) error {
	return r.db.InTx(ctx, func(tx pgx.Tx) error {
		if _, err := tx.Exec(ctx, `INSERT INTO users (id,email,display_name,password_hash,created_at,updated_at) VALUES ($1,$2,$3,$4,$5,$5)`, user.ID,user.Email,user.DisplayName,user.PasswordHash,user.CreatedAt); err != nil { return fmt.Errorf("create user: %w",err) }
		if _, err := tx.Exec(ctx, `INSERT INTO organizations (id,name,slug,created_at) VALUES ($1,$2,$3,$4)`,organization.ID,organization.Name,organization.Slug,organization.CreatedAt); err != nil { return fmt.Errorf("create organization: %w",err) }
		if _, err := tx.Exec(ctx, `INSERT INTO memberships (organization_id,user_id,role) VALUES ($1,$2,'owner')`,organization.ID,user.ID); err != nil { return fmt.Errorf("create owner membership: %w",err) }
		return nil
	})
}
...
func (r *AuthRepository) UserByEmail(ctx context.Context, email string) (models.User, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return models.User{}, domain.ErrNotFound }
	if err != nil { return models.User{}, fmt.Errorf("get user by email: %w", err) }
	return user, nil
}
...
func (r *AuthRepository) UserByID(ctx context.Context, id models.ID) (models.User, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return models.User{}, domain.ErrNotFound }
	if err != nil { return models.User{}, fmt.Errorf("get user: %w", err) }
	return user, nil
}
...
func (r *AuthRepository) UserBySession(ctx context.Context, tokenHash string) (models.User, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return models.User{}, domain.ErrUnauthorized }
	if err != nil { return models.User{}, fmt.Errorf("get session: %w", err) }
	return user, nil
}
...
func (r *AuthRepository) DeleteSession(ctx context.Context, tokenHash string) error {
	...
	if err != nil { return fmt.Errorf("delete session: %w", err) }
	return nil
}
