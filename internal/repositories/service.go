...
type Store interface {
	...
	List(context.Context, models.ID) ([]models.Repository, error)
}
...
func NewService(store Store, projects Projects, workspaceRoot string) *Service {
	return &Service{store: store, projects: projects, workspaceRoot: workspaceRoot, now: time.Now}
}
...
func (s *Service) Create(ctx context.Context, actor models.User, projectID models.ID, input CreateInput) (models.Repository, error) {
	...
	if err != nil || !info.IsDir() {
		return models.Repository{}, domain.Invalid("localPath", "must be an existing directory beneath the workspace root")
	}
	branch, err := gitOutput(ctx, path, "symbolic-ref", "--quiet", "--short", "HEAD")
	if err != nil {
		return models.Repository{}, domain.Invalid("localPath", "must be a Git repository with a checked-out branch")
	}
	repository := models.Repository{ID: models.ID(uuid.NewString()), ProjectID: projectID, Name: strings.TrimSpace(input.Name), LocalPath: path, DefaultBranch: strings.TrimSpace(branch), CreatedAt: s.now().UTC()}
