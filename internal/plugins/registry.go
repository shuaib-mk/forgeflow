...
type RepositoryIntegration interface {
...
func (r *Registrar) WorkflowStep(name string, handler StepHandler) error {
	if name == "" || handler == nil {
		return errors.New("plugin step requires a name and handler")
	}
	if _, exists := r.steps[name]; exists {
		return fmt.Errorf("plugin step %q already registered", name)
	}
...
func (r *Registrar) Notification(name string, handler NotificationHandler) error {
	if name == "" || handler == nil {
		return errors.New("notification handler requires a name and implementation")
	}
	if _, exists := r.notifications[name]; exists {
		return fmt.Errorf("notification handler %q already registered", name)
	}
...
func (r *Registrar) Analytics(name string, processor AnalyticsProcessor) error {
	if name == "" || processor == nil {
		return errors.New("analytics processor requires a name and implementation")
	}
	if _, exists := r.analytics[name]; exists {
		return fmt.Errorf("analytics processor %q already registered", name)
	}
...
func (r *Registrar) Repository(name string, integration RepositoryIntegration) error {
	if name == "" || integration == nil {
		return errors.New("repository integration requires a name and implementation")
	}
	if _, exists := r.repositories[name]; exists {
		return fmt.Errorf("repository integration %q already registered", name)
	}
...
func NewRegistry() *Registry {
	return &Registry{plugins: map[string]Plugin{}, steps: map[string]StepHandler{}}
}
...
func (r *Registry) Register(plugin Plugin) error {
	...
	for name := range registrar.steps {
		if _, exists := r.steps[name]; exists {
			return fmt.Errorf("workflow step %q conflicts with another plugin", name)
		}
	}
	r.plugins[manifest.Name] = plugin
	for name, handler := range registrar.steps {
		r.steps[name] = handler
	}
	return nil
}
